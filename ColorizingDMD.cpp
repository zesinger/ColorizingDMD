...
* 
* Uses GDI+ for BMP file load and save
* 
* You can edit the frames in GIMP external editor if installed in default directory (GIMP is free, it is easy to edit paletted pictures and their palette using it)
* To bring the palette tool, just follow menu Windows → Dockable Dialogs → Colormap then you'll have the indexed color tool available.
* As we use 64 color pictures, only the 64 first colors of the image will be used, don't use the 192 ones remaining
*/
...
/*        char tpath[MAX_PATH];
    ...
    UINT lengthheader = 10 * sizeof(UINT);
    ...
    //fwrite(MycRom.SpriteDetectDwords, sizeof(UINT32), MycRom.nSprites, pfile);
    //fwrite(MycRom.SpriteDetectDwordPos, sizeof(UINT16), MycRom.nSprites, pfile);
    for (UINT ti = 0; ti < MycRom.nFrames; ti++)
    {
        pactiveframes[ti] = 1;
...
/*    char tbuf[MAX_PATH];
    ...
    //MycRom.SpriteDetectDwords = (UINT32*)malloc(MycRom.nSprites * sizeof(UINT32));
    //MycRom.SpriteDetectDwordPos = (UINT16*)malloc(MycRom.nSprites * sizeof(UINT16));
    ...
    if ((!MycRom.ShapeCompMode) || (!MycRom.HashCode) || (!MycRom.CompMaskID) || (!MycRom.MovRctID) || (!MycRom.CompMasks) ||
        ...
        //(!MycRom.SpriteDetectDwords) || (!MycRom.SpriteDetectDwordPos) ||
        (!MycRom.ColorRotations) || (!MycRom.SpriteDetDwords) || (!MycRom.SpriteDetDwordPos)||(!MycRom.SpriteDetAreas))
    {
    ...
    //fread(MycRom.SpriteDetectDwords, sizeof(UINT32), MycRom.nSprites, pfile);
    //fread(MycRom.SpriteDetectDwordPos, sizeof(UINT16), MycRom.nSprites, pfile);
...
HWND hListBox, hwndButtonU, hwndButtonD;
LRESULT CALLBACK MovSecProc(HWND hWin, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        hwndButtonU = CreateWindow(L"BUTTON", L"UP", WS_TABSTOP | WS_VISIBLE | WS_CHILD, 230, 240, 50, 50, hWin, NULL, hInst, NULL);      // Pointer not needed.
        if (!hwndButtonU)
        {
        ...
        hwndButtonD = CreateWindow(L"BUTTON", L"DOWN", WS_TABSTOP | WS_VISIBLE | WS_CHILD, 230, 310, 50, 50, hWin, NULL, hInst, NULL);      // Pointer not needed.
        if (!hwndButtonD)
        {
            ...
            switch (LOWORD(wParam))
            {
                ...
                case IDC_DELALLSAMEFR:
                {
                    ...
                    if (MessageBox(hDlg, L"CAUTION!!!!! This action can't be undone, are you sure you want to delete all the similar frames?\r\nBe aware that if your comparison masks are not correctly/accurately drawn, you may lost some frames that you still need.", L"Confirm", MB_YESNO) == IDYES)
                    {
                        ...
                        InitColorRotation();
                        SetDlgItemText(hDlg, IDC_SAMEFRAME, L"0");
                        cprintf("%i frames were deleted as duplicate frames according the comparison masks", nDelFrames);
                    }
                    return TRUE;
                }
                case IDC_DELFRAME:
                ...
                case IDC_SECTIONLIST:
                {
                    ...
                    if ((HIWORD(wParam) == CBN_SELCHANGE) && (acpos < 255))
                    {
